...
- Pressure-corrected engines, mass flow, Isp, staging and stage delta-v. VAB still exposes one engine part per stage; the sim expands `engine_count` into per-instance lifecycle, gimbal, feed, fuel-depletion and torque
...
hardware fixture and a first controlled pitch/atmosphere gate pass, while closed-loop
elevation is only covered by a short deterministic reference program; Starship ascent/EDL
parity, engine-out recovery and full SAS/flap/RCS equivalence remain open. See the
...
- VAB still exposes one engine part per stage. Runtime expands `engine_count` into per-instance `EngineInstanceState` (lifecycle, gimbal, feed and torque); fuel exhaustion cuts delivered thrust without creating a persistent engine failure state, and mesh clusters remain procedural visuals of those instances.
...
- Reentry has windward plasma, tile charring, survivable belly-flop EDL, thermal break-up VFX, per-piece structural breakup and control-loss authority. Powered descent now coasts below the target descent profile and rejects sub-m/s lateral noise before lighting minimum-throttle engines; richer shock/plasma rendering vs IFT references is still pending.
- The normal orbital-return path now seeds a rotating Starbase intercept plane and predicts the moving catch corridor with surface-relative velocity. The deterministic E2E reaches atmospheric entry and materially reduces the footprint error, but a complete physical tower catch is still pending; the catch gates remain strict and are not widened to hide that gap.
- Starship hull is modelled at the real 9 m diameter with procedural steel, weld seams, windward tiles, heat-shield borders, flaps, raceways, payload-door cues, access panels, vent/drain ports, flap leading-edge/tile-seam cues, Raptor clusters, denser liftoff plume/smoke, and refined Super Heavy grid fins with hinge/lattice detail. The Starbase tower has added carriage rails, catch-arm rub rails/rollers/cables and Ship QD cues. Engine startup now has pre-release glow/vapor/flicker, hot-staging has flash/plume VFX, vacuum burns suppress pad-like smoke, and reentry plasma uses heat-flux-driven cap/wake plus first-pass localized nose/belly/flap glow. The initial-flight pass now includes height-relative camera framing, opaque PBR steel, sky-based phase lighting, renderer-aware screen-space effects, layered plume alpha, material-level Starbase LOD fades, and a geographically aligned USDA NAIP + USGS 3DEP terrain stack: detailed regional coverage for the first 10 km plus a lower-frequency 50 km macro bridge through the 12 km ascent view. The scaled Earth handoff completes at 18 km so the 20 km and higher views use one continuous planetary surface. Remaining visual work is fine reference matching, target-GPU tuning and broader collision/detail coverage; see `docs/audits/LOCAL_TERRAIN_REALISM_2026-09-14.md`.
