...
- Mission UX: usable save/load (C1), orbit→deorbit→ENTRY without teleport (C2), and EDL phase-track/cues (C3) are landed this oleada. LEO warp decay (B3) and hot-stage thrust overlap (B2) are landed. Remaining after oleada B+C: control-loss consequences, visual oleada A (reference captures), R5 multi-motor.
