...
- Per-engine lifecycle/gimbal/thermal/feed/failure state is modeled per instance, and per-engine-mount-geometry torque is now modeled (`PartGraph.GetTotalTorque`/`GetPitchYawRollAngularAcceleration`). Differential per-mount TVC commanding is not implemented (`Vessel.Tick` still mirrors one gimbal command to every gimballed mount in a part). `GetTotalTorque` is now wired into `Vessel.Tick` as an attitude disturbance, but only in the `!hasInput` branch — the piloted branch already applies an idealized full-authority estimate for the same commanded gimbal deflection, so adding the live per-mount torque there too would double-count it.
...
- Mission UX: usable save/load (C1), orbit→deorbit→ENTRY without teleport (C2), and EDL phase-track/cues (C3) are landed this oleada. LEO warp decay (B3), hot-stage thrust overlap (B2), structural breakup (B1) and control-loss authority (`ControlAuthority`) are landed. Visual oleada A (plasma phase alpha, flap/nose V1.1, deluge silhouette) is landed. `tools/visual_playtest.sh` now has `--hotstage` and `--reentry-compare` capture modes for IFT side-by-side reference compare (tooling done, reference-image tuning still pending). R5 multi-motor torque is landed; differential per-mount TVC (R5b) is still pending.
