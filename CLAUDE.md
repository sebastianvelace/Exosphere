...
- Per-engine lifecycle/gimbal/thermal/feed/failure state is modeled per instance. Per-mount torque (`PartGraph.GetTotalTorque`) and differential TVC (R5b: `SolveDifferentialGimbal`) are landed; `Vessel.Tick` applies real per-mount torque with and without pilot input. Remaining propulsion gap: R5d (thrust magnitude still averages gimbal across mixed fixed/gimballed clusters).
