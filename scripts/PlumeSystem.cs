...
public partial class PlumeSystem : Node3D
{
    ...
    public void SetupSH(float innerR, float midR, float outerR, float bellY)
    {
        ...
        _shUnits.Add(BuildUnit("SH_Skirt", bellY, mouthR: 1.45f,
            length: 3.2f, count: 33,
        ...
        float activeFraction, int start = 0, int count = -1, int activeCount = -1)
    {
        ...
        for (int index = start; index < end; index++)
        {
            ...
            // Pad-only fountain. Once the stack is airborne this cone becomes the
            // white teardrop the play camera captured at ~50 m.
            if (u.IsSkirt && altitude > 40.0)
                unitFiring = false;
            ...
            if (unitFiring)
            {
                ...
                float seaLevelBroadening = u.IsSuperHeavy ? 1.38f : 1.12f;
