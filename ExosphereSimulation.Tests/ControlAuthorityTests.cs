...
public sealed class ControlAuthorityTests
{
    ...
    [Theory]
    [InlineData("falcon9_block5_standard_2025.json")]
    [InlineData("newglenn_7x2_public_2026.json")]
    public void IntactConventionalLaunchersDoNotReportMissingStarshipFlapsAsDamage(
        string variantFile)
    {
        ...
        var variant = VehicleVariantDefinition.LoadFromJson(
            Path.Combine(root.FullName, "data", "vehicles", variantFile));
        var vessel = variant.Build(catalog).ToVessel(variant.Name);
