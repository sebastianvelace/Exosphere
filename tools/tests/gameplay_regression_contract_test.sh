...
require_pattern ExosphereSimulation/Flight/EntryCorridorGuidance.cs '? liftUp' 'entry corridor keeps lift-up when the target remains ahead'
require_pattern ExosphereSimulation/Flight/EntryCorridorGuidance.cs ': -liftUp' 'entry corridor reverses lift only for predicted overflight'
...
require_pattern scripts/EDLController.cs 'aimAxis = AerodynamicsModel.ComputeLiftUpEntryAxis' 'catch nominal lift-up trajectory'
